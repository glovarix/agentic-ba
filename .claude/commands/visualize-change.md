...
8. Cross-check field, role, and module names against `artefacts/modules/modules.md` so the prototype uses real product terminology.
...
1. Default save path: `artefacts/prototypes/`.
2. If the source CR is a group folder (Rule 8), or the prototype is large enough to reasonably cover several related CRs, ask the user whether to use a single subfolder — `artefacts/prototypes/{feature-slug}/` — for all related prototype files, instead of separate top-level files. Wait for confirmation before choosing.
