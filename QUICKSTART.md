...
- **Any codebase, any stack — and every integration is off until you turn it on.** Baxter assumes nothing about your language, framework, folder layout, repository host, or issue tracker; it reads your codebase to work out where things live. An issue tracker (ClickUp, Jira, Linear, Azure DevOps, GitHub Issues) and GitHub (`gh` CLI, Projects) are switched on in the `integrations` block of `preferences.json` and are **off on a fresh clone**. Keep your own machine's settings in `preferences.local.json`, which is gitignored and never published. Without an integration, a skill notes what it skipped and produces the artefact anyway — no core artefact depends on any of them.
