...
- **A BRD is deliberately non-technical: Who, Why, What.** The roles who use the product, the business case, and each module with its high-level features listed one line each — nothing more. No functional requirements, no acceptance criteria, no rules, nothing technical. That depth belongs in the module's PRD and its CRs, which is exactly why the two documents never overlap. If a product was built without a BRD, `/generate-retrospective-brd [path]` writes one from the codebase.
