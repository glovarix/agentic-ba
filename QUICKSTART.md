...
**A partial codebase is completely normal** — from a BA perspective a codebase is never finished, so put in whatever you have and Baxter analyses it as it is, flagging what it cannot see rather than inventing it. You never wait until it is "ready". If you have not built the registries yet, run `/generate-module-registry` and `/generate-role-registry`; if there is no BRD, `/generate-retrospective-brd` writes one from the code. *Genuinely nothing built at all?* Start with a BRD, the one artefact written before code exists, and Baxter can walk its features and propose a candidate Change Request from each — but neither registry can be built from a BRD, so module and role names stay unverified until there is code. See **Where are you starting from?** in the README.
...
- **Any codebase, any stack — and every integration is off until you turn it on.** Baxter assumes nothing about your language, framework, folder layout, repository host, or issue tracker; it reads your codebase to work out where things live. An issue tracker (ClickUp, Jira, Linear, Azure DevOps, GitHub Issues) and GitHub (`gh` CLI, Projects) are switched on in the `integrations` block of `preferences.json`, your own real settings file — copy it from the committed `preferences.sample.json` template (every toggle on, for reference) and it stays gitignored and never published. Without an integration, a skill notes what it skipped and produces the artefact anyway — no core artefact depends on any of them.
