...
Three power tools together document the AI feature set at a level of detail beyond a single Product Documentation (PD) artefact — see Rule 0 and Rule 18. Their output is a category of product documentation, so it is nested under `artefacts/product-docs/ai-feature-analysis/`:
...
| AI Feature Registry | `artefacts/product-docs/ai-feature-analysis/` + `context/` | `ai-features.md` in both locations, overwritten on each `/generate-ai-feature-registry` run |
| AI Feature Module Map | `artefacts/product-docs/ai-feature-analysis/` | `ai-feature-module-map.csv`, overwritten (or updated per feature) on each `/generate-ai-feature-dependency-map` run |
...
## Rule 18: AI Feature Analysis Tools — Pre-check Before an AI Feature Spec
...
2. `/ai-feature-audit [feature name]` — what data feeds a given feature
3. `/generate-ai-feature-dependency-map` — what modules each feature depends on

**Before drafting an AI Feature Spec, check:**

- Does `artefacts/product-docs/ai-feature-analysis/ai-features.md` exist, and does it already list the feature (if this is a change to an existing one)?
- Has `/ai-feature-audit` been run for this feature — is there a saved audit in `artefacts/product-docs/ai-feature-analysis/audits/`, or was one produced earlier in this conversation?
- Does `artefacts/product-docs/ai-feature-analysis/ai-feature-module-map.csv` exist and list this feature?
