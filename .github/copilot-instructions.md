...
| `sampleDataFormat` | `"json"` | Output format for `/generate-samples`. `"json"` — always output JSON regardless of codebase type. `"sql"` — output SQL (`BEGIN/COMMIT`, `ON CONFLICT DO NOTHING`) for relational database codebases. |
...
| `{ext}` | `.json` by default; `.sql` if `sampleDataFormat` is set to `"sql"` in `preferences.json` |
...
3. Determine the output format from `preferences.json`:
   - `"json"` (default) → always output `.json` regardless of codebase type
   - `"sql"` → output `.sql` using `BEGIN/COMMIT` and `ON CONFLICT DO NOTHING` — only use this for relational database codebases; if the codebase has no SQL schema, fall back to JSON and flag it to the user
