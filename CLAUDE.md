...
Sample data records are generated by the `/generate-samples` power tool (command file: `.claude/commands/generate-samples.md`, full rules in Rule 12). They live in `artefacts/sample-data/` and follow the naming pattern `sample-{app-slug}-{NN}-{slug}.{ext}`. The `sample-` prefix ensures they are committed to git under the default `commitArtefacts: false` setting.
