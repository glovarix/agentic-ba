...
The module registry is generated by `/generate-module-registry`. It is saved to two locations and uses `templates/MR.md` as its template.
...
| 11 | Release Validation (RV) | Release notes (path in `docs/`) + two branch snapshots in `coderepo/branches/` + sprint number | GitHub issues via `gh` CLI | Yes — full staged vs production diff with undocumented items report |

I'll always confirm the artefact type before writing. You can reply with the number, the acronym, or "proceed".

---

**Power tools — automated skills that do the heavy lifting for you.**
...
| --- | --- | --- | --- |
| 0 | "release validation", "validate the release", "what's on staging", "compare staging", "staging vs production", "what's going to production", "not in the release notes", "release notes" + branch comparison intent | Release Validation (RV) | — (see Rule 15) |
...
Triggered when the user provides release notes and asks to compare what is on staging against production — to confirm which release note items are present in staging, identify anything going to production that is not in the release notes, and surface undocumented changes.
...
**Purpose:** Produce a standard pre-release notes document — a numbered table of release items with descriptions, GitHub issue references, ClickUp card links, and a blank Video column — saved to `docs/Pre-release Sprint {N}/` as both a markdown file and a PDF.
...
5. Group issues by module area and order logically: user-facing areas first, infrastructure last.
...
| "what's on staging and not in production", "validate the sprint 95 release", "compare staging vs prod" | Release Validation (RV) | — (Rule 15) |
