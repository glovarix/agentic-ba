...
| Sample Data | `artefacts/other/sample-data/` | `sample-{app-slug}-{NN}-{slug}.json` |
...
6. Save to `artefacts/other/sample-data/` using the pattern `sample-{app-slug}-{NN}-{slug}.json` where `NN` is zero-padded (01, 02, 03).
...
1. Resolve the target folder:
   ...
   - If no argument is provided, list folders under `artefacts/other/test-suites/` and ask the user to select one.
...
| `/generate-test-plan artefacts/other/test-suites/SERVICES` | → Rule 13 | `.claude/commands/generate-test-plan.md` |
