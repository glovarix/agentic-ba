...
| `includeAcceptanceCriteria` | `true` | `true` — include an Acceptance Criteria section in Change Request (CR), Bug Report (BR), and AI Feature Spec (AI) artefacts where applicable. `false` — omit it from all three. |
...
The role registry is the module registry's counterpart for people: the authoritative list of the product's **default roles** — the roles built into the application, defined in code and present in every deployment before anyone configures anything. It is generated by the `/generate-role-registry` power skill and saved to two locations. Its output structure is defined in `.claude/commands/generate-role-registry.md` — not in `templates/`, which holds core skill templates only.

| File | Purpose |
| --- | --- |
| `artefacts/role-registry/roles.md` | Working artefact copy — never committed to git; `artefacts/` contents never reach GitHub. |
| `context/roles.md` | Working reference copy — available to the team without opening artefacts. |

**The codebase is the only source of a role — there is no other one.** Not a BRD, not a permissions spreadsheet, not an existing artefact, not the user's own description of who uses the product. If a role is not evidenced in `coderepo/`, it does not enter the registry and it does not appear in an artefact. This is stricter than the module registry deliberately: a role name that is wrong, or a role that does not actually exist, silently corrupts every access statement that uses it, and nothing downstream can catch it. `/generate-role-registry` therefore has no provisional mode and does not run without code.

**Default roles only.** A default role ships with the product. A role a customer creates at runtime through an admin screen is configuration in one deployment, not product structure, and never belongs in the registry or in an artefact's role columns. Permissions, job titles held as data, account states, service accounts, and subscription tiers are not roles either — `.claude/commands/generate-role-registry.md` Step 4 has the full exclusion list.

**Always re-read before acting (mandatory):** Read `artefacts/role-registry/roles.md` before generating any artefact that states who can do what. If that file does not exist, try `context/roles.md`. Never rely on a version from earlier in the conversation — the user may have edited it since.

**Every role named in any artefact must come from this registry, spelled exactly as the registry spells it, with its code identifier cited alongside.** Write it as Display Name (`code_identifier`) — the registry's Role name, then its Code Identifier verbatim in backticks — the first time the role appears in each table or section. Thereafter the display name alone is enough within that same table or section. This applies to the PD's Features by Role table above all (Rule 27), and equally to a BRD's Who section, a PRD's role-based requirements, a CR or BR's role references, and a TC's Role-Based cases. The code identifier is what makes a role claim checkable — a display name on its own can be plausibly wrong and nothing downstream catches it. Never invent a role, never infer one from a job title in the user's request, and never carry a role name over from another project or a generic industry equivalent. If the user names a role the registry does not have, flag it in the Sanity Check (Rule 4) rather than silently adopting or correcting it. If a registry row carries `(user-corrected — not located in code)` and so has no code identifier, name the role and mark it unverified in the Sanity Check rather than inventing an identifier for it.

**If the registry is missing (neither file exists):** do not guess, and do not substitute another source. For a PD, follow Rule 27 — it must not be drafted with unverified roles. For any other artefact, read the roles you need directly out of `coderepo/` for that artefact only, state in the Sanity Check that they came from code rather than a registry, and mention once that `/generate-role-registry` would make them verifiable across every future artefact. If there is no codebase either, Rule 4 already governs: state plainly that no role could be verified, and name every role in the artefact as unverified rather than presenting it as fact.
...
| 3 | PD — Product Documentation | The module or product area to document | Codebase — how the module is actually implemented, after its CRs have been built, `artefacts/module-registry/modules.md`, `artefacts/role-registry/roles.md` for the access columns, linked BRDs and TIPs | Yes |
...
**Not sure where to start?** There are only two answers. **You have a codebase** — put it in `coderepo/` and paste your request; a partial codebase is completely normal and I analyse whatever is there as it is. Run `/generate-module-registry` first if you have not built the registry yet, and `/generate-retrospective-brd` if there is no BRD — neither is a prerequisite. **You have no codebase at all** — write a BRD first, and I can propose a candidate CR per feature from it (Rule 26). The module and role registries both need code and wait until there is some. See Rule 25.
...
| `/generate-module-registry` | Nothing — just run it. Needs a codebase; there is no other source | Reads every file in `coderepo/`, identifies modules, areas, and features, folds submodules/CRUD actions/dashboards/standalone AI into their parent module, and derives each module's filename slug | `artefacts/module-registry/modules.md` — the module registry used by all other artefacts |
| `/generate-role-registry` | Nothing — just run it. Needs a codebase; there is no other source | Reads the codebase's access model — role definitions, seed data, permission enforcement, role-based UI branching, the role-management screen — and identifies every default (built-in) role, its code identifier, scope, and out-of-the-box access, excluding customer-created custom roles, permissions, job titles, and service accounts | `artefacts/role-registry/roles.md` + `context/roles.md` — the role registry used for every PD access column and every role named in any artefact |
...
- **Acceptance criteria inclusion (CR, BR, and AI):** When `includeAcceptanceCriteria` is `true` (default), include an Acceptance Criteria section in Change Request (CR), Bug Report (BR), and AI Feature Spec (AI) artefacts where applicable. When `false`, omit it from all three. This setting does not affect other artefact types — a BRD or TC always includes its acceptance criteria regardless.
...
- **No code references in the artefact body.** File paths, table names, field names, route paths, and permission strings must never appear in the artefact itself — not in the checklist, not in the user story, not in the Technical Notes. The artefact is written for BAs, product managers, and QA — not developers reading code. All code-level findings go exclusively in the Sanity Check section.
  - **One exception: a role's code identifier.** A role is always written Display Name (`code_identifier`) on first appearance in a table or section, per the Role Registry section. This is not a code reference in the sense above — it is the check that stops a wrong role name passing silently, and a wrong role name corrupts every access statement built on it. It is deliberately the only code-level string permitted in an artefact body. It does not license a permission string, a table name, or a field name to appear alongside it.
...
- **Title genericness (CR and BR, mandatory).** The part after the module prefix stays short and high-level — a label a person scans in a list, not a summary of the mechanism or full scope. Push specifics (fields, exact behaviour, rationale) down into Summary, Problem & Context, and the In Scope checklist; never load them into the title. This applies to every CR and BR, single-module or grouped — a grouped/master issue spanning more than one module additionally uses a neutral prefix like `[Cross-Module]` rather than naming any one of them (see Rule 8).
...
| Role Registry | `artefacts/role-registry/` + `context/` | `roles.md` in both locations, overwritten on each `/generate-role-registry` run |
...
**PD does not read PRD as a generation input.** When drafting a PD, only read the codebase (plus `artefacts/module-registry/modules.md`, `artefacts/role-registry/roles.md`, and linked BRDs/TIPs, as normal) — never the module's PRD. PD's whole value is that it describes the module strictly as implemented today, independent of what was requested; if PD generation were informed by PRD's content, drift between "what was asked" and "what got built" would get smoothed over instead of surfaced, and TC generation's PRD-vs-PD comparison (below) would lose its meaning. The simplified PD template has no Linked Artefacts table and no PRD cross-reference — there is nowhere in a PD for a PRD to appear at all, so this separation is now structural rather than a rule to remember. See Rule 27.
...
## Rule 25: Where the User Is Starting From — Codebase or No Codebase

This framework is built around `coderepo/`, deliberately: the sanity check, the module names, and every feasibility call come from reading real code.

**There are only two situations, not a spectrum of readiness.** Either there is a codebase to analyse or there is not. From a BA perspective a codebase is never *finished* — it is always partly built — so a partial codebase is the normal, permanent condition and never a reason to hold back. Analyse whatever is in `coderepo/` exactly as it is.

**Expect a codebase.** Almost every project using this framework has one. Case B is a genuine minority: never treat an empty `coderepo/` as the expected starting state, and never volunteer the no-codebase path to someone who plainly has code.

---

**Case A — there is a codebase (the normal case).** Read it and work. Do not qualify, delay, or apologise for what it does not yet contain:

- Verify everything that exists. Flag anything that does not — an unverifiable field, module, role, or route is reported per Rule 4, and a genuinely new module is marked `(new module)` per Rule 9. Never invent, and never imply a check that did not run.
- Coverage tracking the codebase is the design, not a shortfall. If the user seems to expect more, say once that whatever is there is analysed as it is and coverage grows with the code.
- **If `artefacts/business-requirements/` is empty** and the request would benefit from a BRD's context — a PRD, a PD, a large CR, or any request naming product goals rather than a specific change — mention `/generate-retrospective-brd` once: it reads the codebase and works backwards to a client-facing BRD, needing nothing else to exist first. Do not offer it for a small bug report or a one-line change. This is a situational nudge, not a prerequisite.
- **If `artefacts/module-registry/modules.md` is missing**, the Module Registry section already governs what to do. Nothing here overrides it.

---

**Case B — there is no codebase at all (the exception).** Rule 4 already requires stating plainly that verification could not run and listing everything that could not be confirmed. Add to that, at most once per session:

- A BRD is usually the right first artefact, because it is the one artefact written before code exists.
- Once a BRD exists, Rule 26 is the next step: it can propose a candidate CR per feature from it. The module and role registries are not available yet — both are built from code only, and neither has a provisional mode.
- Adding even a starter scaffold or boilerplate for the chosen stack restores part of the check — it anchors folder layout, auth model, and data-layer conventions, so implementation plans and data-model checks become meaningful. Be honest that it will not contain their modules, so module and field names stay unverifiable until they are real.

**Case B1 — a CR or BR is requested with no codebase and no BRD. Ask before drafting.**

A Change Request describes a change to something. With no codebase there is nothing to check feasibility, module names, or conflicts against, and with no BRD there is nothing saying what the product is meant to do either — so the artefact would rest on nothing but the sentence the user just typed. Do not silently draft it. Say so plainly and give the two routes that fix it:

> "A CR is normally checked against your codebase — feasibility, module names, conflicts with what already exists — and there is nothing in `coderepo/` to check against. Two ways forward: **add your codebase** (even a starter scaffold helps, and a partial one is fine) so I can verify this properly, or if nothing is built yet, **start with a BRD** describing what you are building — I can then propose CRs from its features prospectively, this one included. Or say **proceed anyway** and I will draft it with the Sanity Check stating that nothing could be verified."

- Accept any of the three. On **proceed anyway**, draft it normally and make the Sanity Check say explicitly that no codebase and no BRD were available, so feasibility, module names, and conflicts are all unverified.
...
- **If a BRD does exist**, this does not apply — go to Rule 26 and propose CRs from its features instead, noting that feasibility stays unverified until code exists.
- **If `coderepo/` has anything in it at all**, this does not apply either. That is Case A: draft against what is there and flag the rest.
- Ask once per session, not per CR. Once the user has chosen, respect that choice for the rest of the session.

---

Give all guidance above in one or two sentences, at the moment it helps. Case B1 is the only one that asks before drafting; everything else informs and then proceeds. Never suggest the user install, host, or subscribe to anything, and never suggest abandoning the request in favour of a different artefact.

---

## Rule 26: After a BRD — Offer Candidate CRs

Applies mainly to a project with no codebase yet — Case B in Rule 25 — though it works equally on an existing BRD when the user asks for it.

A BRD already contains the raw material for a Change Request list. Section 4 (What) gives each module's features one line each — candidate CRs in all but name. This rule turns that into an offer, so a team with no code yet has somewhere to go next.

**A BRD cannot seed the module registry.** Both registries are built from code alone and neither has a provisional mode — `/generate-module-registry` and `/generate-role-registry` do not run without a codebase. If the user asks for either at this point, say plainly that they wait until there is code, and that module names in these early CRs are therefore unverified. Do not offer a provisional substitute.

**When to offer.** Once a BRD has been saved, or when the user asks for this against an existing BRD. Offer once, then stop and wait:

> "That BRD names {M} features. I can walk them one at a time and pick which become Change Requests. Want to?"

If the user declines or ignores it, do not raise it again in the session.

**How to run it.** Use the `/brainstorm-change` interaction pattern exactly as defined in `.claude/commands/brainstorm-change.md` — that skill already solves "propose many, draft only what is confirmed", and this must not invent a second way of doing it:
...
## Rule 27: PD Generation — Simplified Template, Default Roles Only

Applies every time a Product Documentation (PD) artefact is generated (Rule 1, priority 2).

`templates/PD-Product-Documentation.md` is deliberately short: a compact header, Overview, App Flow, Features by Role, Module Connections, Notifications & Triggers, and Known Limitations. It replaced a longer eight-section version, so **ignore any recollection of numbered sections, per-module subsections, a User Roles table, Key Workflows, an Integrations table, or a Linked Artefacts table** — none of those exist any more. Use the template as it is on disk, every time (Rule 6), and never reintroduce a removed section because an older PD in `artefacts/product-documentation/` still has it.

The compression is the point: a PD is the current, scannable picture of what a module does and who can do it. Depth belongs elsewhere — requirements in the module's PRD, implementation in a TIP, test detail in TCs. Do not restore length by expanding a section beyond its purpose.

**Roles in a PD must be the product's default roles, and nothing else.** This is what makes a PD accurate, so treat it as a hard constraint rather than a preference:

- **Read `artefacts/role-registry/roles.md` before drafting any PD** (falling back to `context/roles.md`), exactly as the module registry is read — every time, never from conversation memory.
- **The Features by Role columns are the registry's roles**, spelled exactly as the registry spells them, each column header written Display Name (`code_identifier`). The template shows three placeholder columns; use as many columns as the product actually has roles, not three. Never pad the table out to the placeholder count, and never drop or truncate a role to fit.
- **Never invent, infer, or import a role.** Not from a job title in the user's request, not from a persona in a BRD, not from a customer's custom role, not from a generic industry equivalent, and not from another project. The codebase is the only thing that can put a role in a PD. If a role is not a default role in this codebase, it does not get a column.
- **Every cell is evidenced.** Each role's access to each feature comes from what the codebase actually enforces — a `—` where a role has no access is a finding, not a gap to fill. Where the code genuinely does not settle it, write `(placeholder — [Team] to confirm)` in that cell rather than guessing, and flag it in the Sanity Check.
- **The Overview's `**Default roles:**` line lists the registry roles that apply to this module** — the same names with their code identifiers, no additions. `**Related modules:**` comes from `artefacts/module-registry/modules.md` the same way, using the registry's module names and groupings exactly.
- **Role names elsewhere in the PD follow the same rule** — the App Flow diagram, Notifications & Triggers, and Known Limitations all use registry names only. Since the Overview and the Features by Role table have already paired each role with its code identifier, the display name alone is enough in these later sections; a diagram node in particular stays readable with the name only.

**If the role registry does not exist,** stop and run `/generate-role-registry` first — it reads the codebase's access model directly. Say so once and proceed with it:

> "A PD's access columns must be the product's built-in roles, and there is no role registry yet. I'll run `/generate-role-registry` first — it reads the roles straight out of the codebase — then draft the PD from it."

**Do not offer to have the roles typed in instead of reading the code** — that is the unverifiable input this rule exists to prevent. But the registry is editable and the user's corrections are always applied: if they name a role, treat it as a pointer to something real, look for it in the code, and record it per `/generate-role-registry` Step 8.

**If there is no codebase at all**, a PD cannot be drafted accurately: it documents what was built, so with nothing built there is nothing to document. Say that plainly rather than producing one from description alone.
...
| "document the orders module" | PD | `templates/PD-Product-Documentation.md` (Rule 27) |
| `/generate-role-registry` / "what roles does the app have", "extract the built-in roles", "role registry" | Role registry generation — power skill | `.claude/commands/generate-role-registry.md` |
