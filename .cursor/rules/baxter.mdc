...
The module registry is an artefact generated by `/generate-modules`. It lives at `artefacts/modules.md` and uses `templates/other/MR.md` as its template.

| File | Purpose |
| --- | --- |
| `artefacts/modules.md` | Product module registry — every named module, feature area, or screen. Used to verify module names in every artefact. |

**Always re-read before acting (mandatory):** If `artefacts/modules.md` exists, read the current saved version before generating any artefact. Never rely on a version from earlier in the conversation — the user may have edited it since. If the file does not exist, proceed without it and note any module names that could not be verified.
...
| 2 | Product Documentation (PD) | The module or product area to document | Codebase, `artefacts/modules.md`, linked BRDs and TIPs | Yes |
| 3 | Technical Implementation Plan (TIP) | The linked issue (or paste its contents) | Codebase, `artefacts/modules.md`, linked BRD | Yes — includes feasibility and data model check |
| 4 | Test Cases (TC) | The linked BRD or feature name | Linked BRD (FRs and ACs), codebase, `artefacts/modules.md` | Yes |
| 5 | AI Feature Spec (AI) | Description of the AI capability needed | Linked BRD, codebase | Yes |
| 6 | Bug Report (BR) | What happened, what you expected, and how to reproduce it | Codebase, `artefacts/modules.md` | Yes — confirms whether behaviour is a genuine bug |
| 7 | Change Request (CR) | Description of what you want to add or change | Codebase, `artefacts/modules.md`, linked BRDs | Yes — checks feasibility and conflicts |
| 8 | Diagram (DIA) | Description of the flow or system to diagram + linked CR or issue or BRD | Linked issue, artefact, codebase, `artefacts/modules.md` | Yes — checks flows and states match the real codebase |
...
| 10 | Entity Relationship Diagram (ERD) | Description of which tables to include + linked BRD, CR, or TIP | Codebase schema, `artefacts/modules.md` | Yes — verifies table names, column names, and relationships against the codebase |
...
| --- | --- |
| `/generate-modules` | Scans the codebase and builds a product module registry at `artefacts/modules.md` |
...
1. **Names** — module names, field names, role names, route paths. Correct any that do not match `coderepo/` or `artefacts/modules.md`.
...
| Module Registry | `artefacts/` | `modules.md` (single file, overwritten on each `/generate-modules` run) |
...
## Rule 9: Module Registry — Keep `artefacts/modules.md` in Sync

Triggered automatically whenever a CR, BR, or AI artefact introduces a **brand new module** — one that does not already appear in `artefacts/modules.md`.

**What counts as a new module:** a named product area, feature section, or screen that the request explicitly treats as a standalone module and that is not listed under any section of `artefacts/modules.md`.

**Steps:**

1. After generating the artefact, scan `artefacts/modules.md` for the module name(s) mentioned in the issue.
2. If every module is already listed — do nothing. No mention needed.
3. If one or more modules are missing, propose the addition inline — for example: "The artefact references **{Module Name}**, which is not yet in `artefacts/modules.md`. I recommend adding it under section **{best-fit section}**. Shall I update the file?"
4. Wait for the user's confirmation before writing.
5. On confirmation, add the module to the correct section table in `artefacts/modules.md` — module name and a brief notes entry. Do not restructure existing sections.

**Do not** update `artefacts/modules.md` speculatively or for modules that already exist under a different name or grouping. If in doubt, flag it and ask rather than edit silently.
